...
def foward_loop():
    ...
    B = 0
    # 1秒ごとの制御ループを開始
    for _ in range(1000):  # ループ回数は適宜調整
        if A < 4095:
            A += 64
        elif B < 1536:
            B += 64
...
def gen_com_set_target_value(data_json):
    ...
    '''
    target_position = data_json["target"]["position"]
    target_command = data_json["target"]["command"]
    '''

    # 最初の10ビットから始まるビット列
    bit_list = [identifier_bits + mid_bits]

    for target in data_json["target"]:
        value = int(target["position"]) & 0xFFF  # 12ビット
...
def fixed_motion(data_json):
            # 最初の6ビットは63（111111）、次の4ビットは識別子0000
    identifier_bits = '111111'  # 63の6ビット表現
    if data_json["type"] == "fixed_motion":
        mid_bits = '0000'  # targetの場合の識別子
        if data_json["motion"] == 'crawl':
            print('crawl')
            #foward_loop()
            #bit_list = [identifier_bits + mid_bits]
            #A_bits = f'{A:012b}'
            #B_bits = f'{B:012b}'
            #bit_list.append(A_bits)
            #bit_list.append(B_bits)
            #print(bit_list)
            #return ''.join(bit_list)
            
        elif data_json["motion"] == 'trot':
            print('trot')
        elif data_json["motion"] == 'pace':
            print('pace')
        elif data_json["motion"] == 'bound':
            print('bound')
    else:
        raise ValueError("Unsupported type")
...
def listen_front():
    try:
        ...
        while not stop_event.is_set():                                     # 常に受信待ち
            ...
            elif data_json["type"] == "fixed_motion":
                esp32_com = fixed_motion(data_json)
        ...
        print(f"Error in listen_front: {e}")
...
def data_parse():
    ...
    print('format_value='+ str(format_value))
    # PVC1~4とCGC1~4
...
def convert_to_json():
    ...
    if _parsedData['format'] == 5:
        pvc_data = {
            ...
            "sensors":[
            {"num":0, "position":_parsedData['field1'], "voltage":_parsedData['field2'], "command":_parsedData['field3']} 
        ]
    }
        ...
        pvc_data = {
            "type":"current_sensor_value",
            "sensors":[
            {"num":1, "position":_parsedData['field1'], "voltage":_parsedData['field2'], "command":_parsedData['field3']}
        ]
    }
        ...
        pvc_data = {
            "type":"current_sensor_value",
            "sensors":[
            {"num":2, "position":_parsedData['field1'], "voltage":_parsedData['field2'], "command":_parsedData['field3']} 
        ]
    }
        ...
        pvc_data = {
            "type":"current_sensor_value",
            "sensors":[
            {"num":3, "position":_parsedData['field1'], "voltage":_parsedData['field2'], "command":_parsedData['field3']}
        ]
    }
